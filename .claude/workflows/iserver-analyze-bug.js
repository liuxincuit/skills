...
const JIRA_SCHEMA = {
  type: 'object',
  properties: {
    jiraRaw:  { type: 'string', description: 'Jira 问题的完整原始输出，含 QA 评论' },
    keywords: { type: 'array',  items: { type: 'string' }, description: '5-10 个搜索关键词/短语' },
    summary:  { type: 'string', description: '2-3 句中文缺陷摘要' }
  },
  required: ['jiraRaw', 'keywords', 'summary']
};

const phase1 = await agent(`你是一个缺陷分析助手，需要完成两项任务：

## 任务 1：使用 supermap-jira 技能获取 Jira 问题
找到 supermap-jira SKILL.md 阅读了解使用方法，然后运行脚本读取 Jira 问题：
${issueKey}
...
const jiraRaw     = phase1?.jiraRaw || '(无可用详情)';
const topKeywords = (phase1?.keywords || []).slice(0, 6);
const summaryText = phase1?.summary || `缺陷 ${issueKey} 分析`;

log(`缺陷摘要: ${summaryText}`);
...
const CONCLUSION_SCHEMA = {
  type: 'object',
  properties: {
    verdict:    { type: 'string', enum: ['属于', '不属于', '无法判断'], description: '缺陷有效性结论' },
    conclusion: { type: 'string', description: '完整的中文分析报告，包含要求的 5 个部分' }
  },
  required: ['verdict', 'conclusion']
};

const phase3 = await agent(`你是一名资深技术分析师。请综合所有证据判断缺陷是否真实存在。
...
## 输出要求
conclusion 字段请用中文撰写，包含以下 5 个部分：
...
verdict 字段取 conclusion 第 1 部分的三选一结论。`, {
...
const conclusion = phase3?.conclusion || '未生成结论。';
const verdictRaw = phase3?.verdict || '无法判断';
...
const escapedContent = esc(fileContent);

const writeResult = await agent(`将以下分析结果写入文件。

文件路径: ${outputFile}
（若是相对路径，先用 pwd/realpath 解析为绝对路径，再使用 Write 工具）
...
return {
  ok: true,
  verdict: verdictRaw === '属于' ? 'bug' : verdictRaw === '不属于' ? 'not_bug' : 'uncertain',
  jira_key: issueKey,
