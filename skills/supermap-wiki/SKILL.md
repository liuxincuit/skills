...
- `--no-comments`: 不获取评论
- `--no-images`: 不提取图片
- `--download-images`: 同时把提取到的图片下载到当前工作空间（除非用户指定目录，见下）
- `--download-dir <目录>`: 同时把图片下载到指定目录（优先级高于 `--download-images`；目录不存在自动创建；不同页面同名文件自动加 `-{pageId}` 后缀避免覆盖）

> **下载图片约定**：需要下载图片时，**除非用户说明了保存目录，否则下载到当前工作空间**（`--download-images`），不要自行写 curl/脚本绕过。下载后「页面图片」表中每行会标注本地路径，文件名与下载链接中的原始文件名一致。图片提取链接的 TLS 证书不受信属预期，脚本已处理
