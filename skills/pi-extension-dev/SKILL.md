...
# verify.ts 内 import 扩展文件用绝对路径，如 "/path/to/skills/pi/extensions/compact-tools/index.ts"
...
- 仓库示例：`pi/extensions/compact-tools/index.ts`（同名覆盖 + spawnHook + operations 默认超时）、`pi/extensions/bash-approver/index.ts`（按 sessionId 逐节点注册 authorizer link）
